...
def main(_class, host=None):
    ...
    #if not _class:
    #    print('Enter a switch class. Valid classes;')
    #    print('cisco\narista\nmellanox\nlenovo\n')
    #    _class = input('Enter switch class: ')
    #else:
    #    cfg['class'] = _class

    #if not host:
    #    host = rlinput('Enter host ip address: ', prefill='192.168.32.20')
    #else:
    #    host = cfg['host']
    ...
    if _class:
        ...
        print('Enter a switch class. Valid classes;')
        print('cisco\narista\nmellanox\nlenovo\n')
        _class = rlinput('Enter switch class: ', prefill=_class)
        cfg['class'] = _class
    ...
    if host:
        ...
        try:
            ...
            host = '192.168.32.20'
        host = rlinput('Enter host ipv4 address: ', prefill=host)
        cfg['host'] = host
    ...
    user = rlinput('Enter userid: ', prefill=user)
    cfg['user'] = user
    ...
    passwd = rlinput('Enter passwd: ', prefill=passwd)
    cfg['passwd'] = passwd

    #CSpw123
    #sw = SwitchFactory.factory(_class, host, 'admin', 'admin', mode='active')
    sw = SwitchFactory.factory(_class, host, user, passwd, mode='active')
...
#    parser.add_argument('--class', choices=['cisco', 'mellanox', 'lenovo'],
#                        help='switch class', dest='clas', default=None)
    parser.add_argument('clas', choices=['cisco', 'arista', 'mellanox', 'lenovo'],
                        help='switch class')
